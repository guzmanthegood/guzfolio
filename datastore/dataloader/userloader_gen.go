...
func (l *UserLoader) Load(key uint) (*model.User, error) {
...
func (l *UserLoader) LoadThunk(key uint) func() (*model.User, error) {
	l.mu.Lock()
...
func (l *UserLoader) LoadAll(keys []uint) ([]*model.User, []error) {
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *UserLoader) LoadAllThunk(keys []uint) func() ([]*model.User, []error) {
	results := make([]func() (*model.User, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *UserLoader) Prime(key uint, value *model.User) bool {
	l.mu.Lock()
	var found bool
...
func (l *UserLoader) unsafeSet(key uint, value *model.User) {
	if l.cache == nil {
		l.cache = map[uint]*model.User{}
	}
	l.cache[key] = value
}
